...
def create_session(resource_id: str, user_id: str) -> None:
    ...
    print(f"  Session ID: {remote_session.get('id', remote_session.get('sessionId', 'unknown'))}")
    print(f"  User ID: {remote_session.get('userId', remote_session.get('user_id', user_id))}")
    print(f"  App name: {remote_session.get('appName', remote_session.get('app_name', 'unknown'))}")
    print(f"  Last update time: {remote_session.get('lastUpdateTime', remote_session.get('last_update_time', 'unknown'))}")
...
def list_sessions(resource_id: str, user_id: str) -> None:
    ...
    if isinstance(sessions_response, dict):
        ...
        print(f"Unexpected sessions response: {sessions_response!r}")
        return
...
def _format_event(event: dict) -> str | None:
    """Formats stream events to show readable responses."""
    ...
    for part in parts:
        ...
        if "function_response" in part:
            response = part["function_response"] or {}
            name = response.get("name", "tool")
            payload = response.get("response", {}) or {}
            message = payload.get("message")
            status = payload.get("status")
            if message:
                tool_lines.append(f"[{name}] {message}")
            elif status:
                tool_lines.append(f"[{name}] status: {status}")
