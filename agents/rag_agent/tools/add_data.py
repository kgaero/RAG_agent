...
def add_data(
    ...
    if not check_corpus_exists(corpus_name, tool_context):
        return {
            "status":"error",
            "message":f"Corpus '{corpus_name}' does not exist. Please create it using the create_corpus tool.",
    ...
    try:
        ...
        return {
            ...
            "message": (
                "Successfully added "
                f"{import_result.imported_rag_files_count} file(s) to corpus "
                f"'{corpus_name}'{conversion_msg}"
            ),
            "corpus_name": corpus_name,
